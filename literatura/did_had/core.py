...
class DidHad:
    ...
    def fit(
        ...
        # Map time to ordinal ranks based on sorted unique values, matching R's
        # pdata.frame + make.pbalanced + cur_group_id() behavior. Without this,
        # gaps in the time variable (e.g. a missing year) cause horizons to be
        # silently skipped instead of compressed.
        unique_times = np.sort(df[time].unique())
        original_times = unique_times.copy()
        time_to_rank = {t: i + 1 for i, t in enumerate(unique_times)}
        df[time] = df[time].map(time_to_rank).astype(int)
        ...
        t_min = times.min()
        t_max = times.max()
        # Original (pre-ranking) adoption period for display
        adoption_period_original = original_times[F - 1]
        ...
        self._results = DidHadResults(
            ...
            n_groups=n_groups,
            n_periods=n_periods,
            adoption_period=adoption_period_original,
            kernel=self.kernel,
            alpha=self.alpha,
            dynamic=dynamic,
    ...
    def _compute_effect(
        ...
        # Match R's did_had_est: count observations with D <= h_star (inclusive)
        # rather than rely on nprobust's N.h, which can differ at the boundary.
        n_in_bw = int(np.sum(dose_use <= bw_eff))
