...
data temp;
  set scftestr.scfp1992;
  rename x7020=singlePEU;
  ...
  *gaps = (5.50 - sinc - ssinc) / 5.50;
  gapr = (rinc + rsinc - 4.30) / 4.30;
  gaps = (sinc + ssinc - 4.30) / 4.30;
...
data scfout.regressiondata92;
  set temp;
  keep wageincome financialassets durabledebt financialdebt housedebt homeeq wgt minwage educationdebt creditcard
       havecc veh_inst minwageworker singlePEU havecc totaldebt totalassets nonfinancialassets vehicles otherinstall otherassets salaryt age year;
...
PROC EXPORT DATA=scfout.regressiondata92
            OUTFILE="C:\research\minwage\SCF\scfregdata92.csv"
            DBMS=csv REPLACE;
RUN;
