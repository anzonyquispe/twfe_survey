...
data temp;
  ...
  *gaps = (5.50 - sinc - ssinc) / 5.50;
  gapr = (rinc + rsinc - 6.30) /6.30;
  gaps = (sinc + ssinc - 6.30) /6.30;
...
data scfout.regressiondata07;
  set temp;
  keep wageincome financialassets durabledebt financialdebt housedebt homeeq wgt minwage educationdebt creditcard 
       havecc veh_inst minwageworker singlePEU totaldebt totalassets nonfinancialassets vehicles otherinstall otherassets salaryt age year;
...
PROC EXPORT DATA=scfout.regressiondata07
            OUTFILE="C:\research\minwage\SCF\scfregdata07.csv"
            DBMS=csv REPLACE;
RUN;
