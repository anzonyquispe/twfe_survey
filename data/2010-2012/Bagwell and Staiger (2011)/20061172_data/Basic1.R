...
# Table 3a 
# Table 4, just add "MisMatch" (referred to as "outsiders" in the paper) as a regressor
# Table 5c, just take define Import=log(import) (Idem for Appendix, Tables 3a, upper-left panel and 3b, upper panel)
# Appendix, Table 3a, lower panel; just exclude Chinese Observations, ie, NewmemberAll<-subset(NewmemberAll,Country!="China")
# First part of the code is for regressions by-industry; second part is for regressions by-country

rm(list=ls(all=TRUE))

#Load in necessary libraries.  Survival is needed for the 
#Tobit regression (from a generalized survival regression.
#'car' is the "companion to applied regression"
...
write.table(cd,"C:/TOT_Project/Estimation Code/Baseline Estimates/Results/Basic1.TXT",row.names=FALSE, col.names = FALSE, append=FALSE)
...
#eliminate observations without tariff bindings
...
NewmemberAll0<-NewmemberAll

Id<-as.factor(NewmemberAll0$Product)    #Product ID HS6,as a factor
Country<-NewmemberAll0$Country          #Country
Import<-(NewmemberAll0$Import/1000)     #Average imports in Millions
                        #Pre-accession MFN Tariff
...
IDCHAR_pre<-as.character(Id.num)        #to allow for string manipulations
                                        #turn ID into a character         
IDCHAR<-ifelse(nchar(IDCHAR_pre)>5,IDCHAR_pre,paste("0",IDCHAR_pre,sep=""))  
                        #if the text string only has five characters, 
                        #it means the leading zero was
                        #missing in the numeric variable.  
                        #Add the zero so that all six-digit
                        #classifications have 6 digits
                        
HSCLASS<-as.numeric(substr(IDCHAR,1,h))     #Take the first 2 characters to define HS2 industries
HS<-as.factor(HSCLASS)              #turn this into a factor, which is treated as a 
                                    #fixed effect in the regressions
MisMatch<-(NewmemberAll0$MisMatch/1000)       #Mismatch ("Outsiders") variable -- just add it to the regressions
...
#Remove observations which have unavailable tariff or import data 
...
#subroutine for cleaning.  Here, we drop industries if there is less than 2 observations.  
...
#calculate the variables is the initial MFN tariff.  
#If there is none (the case of Kyrg, and some refined industries), then do not include it.
...
#write the data to file
    ...
    write.table(cd2,"C:/TOT_Project/Estimation Code/Baseline Estimates/Results/Basic1.TXT",row.names=FALSE, col.names = FALSE, append=TRUE)

#in the row below, write the standard errors.  There is a 
#negative sign placed in from the of the standard errors
#so that excel can be used to place parentheses around 
#the st errors using accounting notation
...
NewmemberAll<-subset(NewmemberAll,is.na(M)==FALSE)
NewmemberAll<-subset(NewmemberAll,M>0)
...
Import<-(NewmemberAll0$Import/1000)
TariffBase<-ifelse(is.na(NewmemberAll0$WMFN),NewmemberAll0$MFN,NewmemberAll0$WMFN)
TariffFinal<-ifelse(is.na(NewmemberAll0$WBND),NewmemberAll0$BND,NewmemberAll0$WBND)
...
MisMatch<-(NewmemberAll0$MisMatch/1000)       #Mismatch variable (not used in this file)
