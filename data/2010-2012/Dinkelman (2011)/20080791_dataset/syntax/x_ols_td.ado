...
cap program drop x_ols_td
program define x_ols_td
	version 6.0
...
/* THIS PART CREATES THE MATRIX FOR OUTPUT */
local z=1;
matrix results = (0);
while `z'<=`xreg'{;
	tempvar se1`z' se2`z';

	matrix beta`z'=_b[`X`z''];
	matrix se`z'=_se[`X`z''];

	local se`z'=_se[`X`z''];
	qui gen `se1`z''=cov_dep[`z',`z'];
	qui gen `se2`z''=sqrt(`se1`z'');
	qui sum `se2`z'';
	matrix se2`z' = (r(mean));

	matrix results=(results\beta`z'\se2`z');
...
matrix results = (results\cut1\cut2\N);

matrix colnames results = `depend';
svmat results;		/* store estimates as variables*/;
keep if results!=.;
keep results;
...
order id results; 
qui ren results `depend'$label;

sort id;
save "$temp\spatialse_$label.dta", replace;
