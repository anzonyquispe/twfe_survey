...
infile using h70.dct, using(../year02/raw/H70.DAT)
sort dupersid
save meps02person
d
sum
...
infile using h67a.dct, using(../year02/raw/H67A.DAT)
...
count
sort dupersid
merge dupersid using meps02person
tab _merge
sort _merge
...
gen dualscript = (pd_medicaid / pd_total) if mcrev02=="1" & mcdev02=="1"
...
gen mcselfscript = (pd_self / pd_total) if mcrev02=="1"
...
* almost always those person have mcrev02=="01" so just stick with
...
* these are defined to match up with IMS data
...
egen meps02paid = sum(wgtpaid), by(shortnam)
rename pd_total avgpaid
rename mcarpaid avgmcarpaid

collapse (mean) mcarscript dualscript mcselfscript avgmcarpaid avgpaid generic brand numscripts meps02paid top500 [aw=personwgt], by(shortnam)

rename mcarscript mcar02mepsrx
rename dualscript dual02mepsrx
rename mcselfscript mcself02mepsrx

gen mcar02mepspd = avgmcarpaid / avgpaid

drop avgmcarpaid avgpaid
rename generic gen02meps
rename brand brand02meps
rename numscripts meps02scripts

label var meps02paid "weighted amt paid / 1000"
label var mcar02mepsrx "meps medicare share - weighted script share"
label var dual02mepsrx "dual share - weighted script share"
label var mcself02mepsrx "meps medicare self share - weighted script share"
label var mcar02mepspd "meps medicare share - weighted $ share"
label var gen02meps "fraction of 02 meps scripts generic"
label var brand02meps "fraction of 02 meps scripts brand"
label var meps02scripts "# of scripts in 2002 meps"
...
label var merge1 "merge of meps 02 to medicaid 03"
