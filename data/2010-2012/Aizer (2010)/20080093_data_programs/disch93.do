...
clear;
cap log close;
log using disch93.log,replace;
set mem 900m;

/* 1993 - A version with individual ages*/

infix county 392-393 agecat 13-15 gender 16 racecat 17-18 source 38-39
type 40-41 ins 53-54 mdchcfa 381-382 str eecode1 356-360 str eecode2
...
mvdriver mvped anyinj batter unint gun cut fight drown strang poison;

collapse (sum) preg* er assault* suicide mv* batter unint anyinj gun cut 
drown strang fight poison, by (fips race age female poor);

sort fips race age female poor;

gen year=1993;

save disch93.dta,replace;
clear;
local z=`z'+1;
};
