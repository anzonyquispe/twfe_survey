...
areg ltaks_sd_min_math katrina_frac_class female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I* if year <= 2005, absorb(campus) cluster(campus)
areg ltaks_sd_min_read katrina_frac_class female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I* if year <= 2005, absorb(campus) cluster(campus)
areg lperc_attn katrina_frac_class female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I* if year <= 2005, absorb(campus) cluster(campus)
areg linfractions katrina_frac_class female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I* if year <= 2005, absorb(campus) cluster(campus)
...
*RUN LINEAR MODEL
  ...
  foreach var of varlist taks_sd_min_math taks_sd_min_read{;
	areg `var'  katrina_frac_class  l`var'_* female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I* if  year <= 2005 & `var'_quartile != ., cluster(campus) absorb(campus);
	outreg2 katrina_frac_class using outreg_quartile_class, excel nocons bdec(2);
  };
  foreach var of varlist perc_attn infractions {;
	areg `var'  katrina_frac_class  l`var'_* female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I* if  year <= 2005, cluster(campus) absorb(campus);
	outreg2 katrina_frac_class using outreg_quartile_class, excel nocons bdec(2);
  };
  # delimit cr
  ...
  *LOOP OVER DEPENDENT VARIABLES;
	...
	foreach var of varlist taks_sd_min_math taks_sd_min_read {;
		areg `var' katrina_frac_class  l`var'_* female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I* 
			if `var'_quartile == `quartile' & year <= 2005, cluster(campus) absorb(campus);
		outreg2 katrina_frac_class using outreg_quartile_class, excel nocons bdec(2);
