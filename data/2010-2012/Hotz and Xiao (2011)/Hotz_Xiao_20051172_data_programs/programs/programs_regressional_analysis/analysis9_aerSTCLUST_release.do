...
local pathpgs3  	"/rdcprojects/la00296/disclosure/July2007/STCLUST/" 
local pathdisclose 	"/rdcprojects/la00296/disclosure/July2007/STCLUST/"
local pathoutside	"/rdcprojects/la00296/data/outside/" 


log using `pathpgs3'analysis9_aerSTCLUST.log, replace 
} 
...
forvalues i=1/`k' {
	...
	reg exit `exit_vars_dc_idv`i'' if year==1987 | year==1992, cluster(num_st)
	* local i1=6*(`i'-1)+1 
	* gen T8_`i1'flag=e(sample)
	outreg using `pathpgs3'exit_idv, se 3aster bracket bdec(3) append
	
	reg exit `exit_vars_dc_idv`i'' year1992 if year==1987 | year==1992, cluster(num_st) 
	* local i2=6*(`i'-1)+2
	* gen T8_`i2'flag=e(sample) 
	outreg using `pathpgs3'exit_idv, se 3aster bracket bdec(3) append
	
	areg exit `exit_vars_dc_idv`i'' if year==1987 | year==1992, absorb(num_st) cluster(num_st) 
	* local i3=6*(`i'-1)+3
	* gen T8_`i3'flag=e(sample) 
	outreg using `pathpgs3'exit_idv, se 3aster bracket bdec(3) append
	
	areg exit `exit_vars_dc_idv`i'' year1992 if year==1987 | year==1992, absorb(num_st) cluster(num_st) 
	* local i4=6*(`i'-1)+4
	* gen T8_`i4'flag=e(sample)
	outreg using `pathpgs3'exit_idv, se 3aster bracket bdec(3) append
	
	xtreg exit `exit_vars_dc_idv`i'' year1992 d_state1-d_state56 if (year==1987 | year==1992) & exit_panel_ein==1, fe i(num_ein) cluster(num_st)
	* local i5=6*(`i'-1)+5
	* gen T8_`i5'flag=e(sample)
	outreg using `pathpgs3'exit_idv, se 3aster bracket bdec(3) append

	xtreg exit `exit_vars_dc_idv`i'' year1992 if (year==1987 | year==1992) & exit_panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* local i6=6*(`i'-1)+6
...
forvalues i=1/`k' {
	...
	reg accredit `vars_dc_idv`i'', cluster(num_st)
	* local i1=6*(`i'-1)+1 
	* gen T11_`i1'flag=e(sample) 
	outreg using `pathpgs3'accredit_idv, se 3aster bracket bdec(3) append
	
	reg accredit `vars_dc_idv`i'' year1992 year1997, cluster(num_st) 
	* local i2=6*(`i'-1)+2
	* gen T11_`i2'flag=e(sample) 
	outreg using `pathpgs3'accredit_idv, se 3aster bracket bdec(3) append
	
	areg accredit `vars_dc_idv`i'', absorb(num_st) cluster(num_st) 
	* local i3=6*(`i'-1)+3
	* gen T11_`i3'flag=e(sample) 
	outreg using `pathpgs3'accredit_idv, se 3aster bracket bdec(3) append
	
	areg accredit `vars_dc_idv`i'' year1992 year1997, absorb(num_st) cluster(num_st) 
	* local i4=6*(`i'-1)+4
	* gen T11_`i4'flag=e(sample) 
	outreg using `pathpgs3'accredit_idv, se 3aster bracket bdec(3) append

	xtreg accredit `vars_dc_idv`i'' year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* local i5=6*(`i'-1)+5
	* gen T11_`i5'flag=e(sample)
	outreg using `pathpgs3'accredit_idv, se 3aster bracket bdec(3) append
	
	xtreg accredit `vars_dc_idv`i'' year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* local i6=6*(`i'-1)+6
...
foreach var of local depvarsofT13 {
	...
	* gen T13_`colT13'_``var'_i0'flag=e(sample) 	
	...
	forvalues i=1/`k' {
		...
		*------------------------------------------------------------*
		reg `var' `vars_dc_idv`i'', cluster(num_st)
		* local `var'_i1=6*(`i'-1)+1 
		* gen T13_`colT13'_``var'_i1'flag=e(sample)
		outreg using `pathpgs3'`var', se 3aster bracket bdec(3) append
		*------------------------------------------------------------*
		reg `var' `vars_dc_idv`i'' year1992 year1997, cluster(num_st) 
		* local `var'_i2=6*(`i'-1)+2
		* gen T13_`colT13'_``var'_i2'flag=e(sample) 
		outreg using `pathpgs3'`var', se 3aster bracket bdec(3) append 
		*------------------------------------------------------------*
		areg `var' `vars_dc_idv`i'', absorb(num_st) cluster(num_st) 
		* local `var'_i3=6*(`i'-1)+3
		* gen T13_`colT13'_``var'_i3'flag=e(sample) 
		outreg using `pathpgs3'`var', se 3aster bracket bdec(3) append
		*------------------------------------------------------------*		 
		areg `var' `vars_dc_idv`i'' year1992 year1997, absorb(num_st) cluster(num_st)
		* local `var'_i4=6*(`i'-1)+4
		* gen T13_`colT13'_``var'_i4'flag=e(sample)
		outreg using `pathpgs3'`var', se 3aster bracket bdec(3) append
		*------------------------------------------------------------*	
		xtreg `var' `vars_dc_idv`i'' year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
		* local `var'_i5=6*(`i'-1)+5
		* gen T13_`colT13'_``var'_i5'flag=e(sample)
		outreg using `pathpgs3'`var', se 3aster bracket bdec(3) append 
		*------------------------------------------------------------*			
		xtreg `var' `vars_dc_idv`i'' year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
...
foreach var of local dep_chain {
	...
	*------------------------------------------------------------*		 
	areg `var' `vars_dc_idv5' scrat_chain educ_chain year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSchain_`var'idv5_1flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv5' scrat_chain educ_chain year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSchain_`var'idv5_2flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv5' scrat_chain educ_chain year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSchain_`var'idv5_3flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg `var' `vars_dc_idv6' scrat_chain no_scrat_chain educ_chain no_educ_chain year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSchain_`var'idv6_1flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv6' scrat_chain no_scrat_chain educ_chain no_educ_chain year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSchain_`var'idv6_2flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv6' scrat_chain no_scrat_chain educ_chain no_educ_chain year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSchain_`var'idv6_3flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		
	areg `var' `vars_dc_idv15' scrat0dc_chain edddc_chain year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSchain_`var'idv15_1flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv15' scrat0dc_chain edddc_chain year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSchain_`var'idv15_2flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv15' scrat0dc_chain edddc_chain year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSchain_`var'idv15_3flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg `var' `vars_dc_idv16' scrat0dc_chain no_scrat0dc_chain edddc_chain no_edddc_chain year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSchain_`var'idv16_1flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	xtreg `var' `vars_dc_idv16' scrat0dc_chain no_scrat0dc_chain edddc_chain no_edddc_chain year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSchain_`var'idv16_2flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv16' scrat0dc_chain no_scrat0dc_chain edddc_chain no_edddc_chain year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
...
* treat exit regression separately
	...
	*------------------------------------------------------------*		 
	areg exit `exit_vars_dc_idv5' scrat_chain educ_chain year1992, absorb(num_st) cluster(num_st)
	* gen TMQSchain_exit_idv5_1flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg exit `exit_vars_dc_idv5' scrat_chain educ_chain year1992 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSchain_exit_idv5_2flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg exit `exit_vars_dc_idv5' scrat_chain educ_chain year1992 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSchain_exit_idv5_3flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg exit `exit_vars_dc_idv6' scrat_chain no_scrat_chain educ_chain no_educ_chain year1992, absorb(num_st) cluster(num_st)
	* gen TMQSchain_exit_idv6_1flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	xtreg exit `exit_vars_dc_idv6' scrat_chain no_scrat_chain educ_chain no_educ_chain year1992 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSchain_exit_idv6_2flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg exit `exit_vars_dc_idv6' scrat_chain no_scrat_chain educ_chain no_educ_chain year1992 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSchain_exit_idv6_3flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg exit `exit_vars_dc_idv15' scrat0dc_chain edddc_chain year1992, absorb(num_st) cluster(num_st)
	* gen TMQSchain_exit_idv15_1flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	xtreg exit `exit_vars_dc_idv15' scrat0dc_chain edddc_chain year1992 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSchain_exit_idv15_2flag=e(sample)	
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg exit `exit_vars_dc_idv15' scrat0dc_chain edddc_chain year1992 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSchain_exit_idv15_3flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg exit `exit_vars_dc_idv16' scrat0dc_chain no_scrat0dc_chain edddc_chain no_edddc_chain year1992, absorb(num_st) cluster(num_st)
	* gen TMQSchain_exit_idv16_1flag=e(sample)	
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*
	xtreg exit `exit_vars_dc_idv16' scrat0dc_chain no_scrat0dc_chain edddc_chain no_edddc_chain year1992 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSchain_exit_idv16_2flag=e(sample)
	outreg using `pathpgs3'MQSchain, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg exit `exit_vars_dc_idv16' scrat0dc_chain no_scrat0dc_chain edddc_chain no_edddc_chain year1992 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
...
foreach var of local dep_entry {
	...
	*------------------------------------------------------------*		 
	areg `var' `vars_dc_idv5' scrat_entry educ_entry year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSentry_`var'idv5_1flag=e(sample)
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv5' scrat_entry educ_entry year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSentry_`var'idv5_2flag=e(sample)
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv5' scrat_entry educ_entry year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSentry_`var'idv5_3flag=e(sample)
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg `var' `vars_dc_idv6' scrat_entry no_scrat_entry educ_entry no_educ_entry year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSentry_`var'idv6_1flag=e(sample)
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv6' scrat_entry no_scrat_entry educ_entry no_educ_entry year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSentry_`var'idv6_2flag=e(sample)
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv6' scrat_entry no_scrat_entry educ_entry no_educ_entry year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSentry_`var'idv6_3flag=e(sample)
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		
	areg `var' `vars_dc_idv15' scrat0dc_entry edddc_entry year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSentry_`var'idv15_1flag=e(sample)
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv15' scrat0dc_entry edddc_entry year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSentry_`var'idv15_2flag=e(sample)
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv15' scrat0dc_entry edddc_entry year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSentry_`var'idv15_3flag=e(sample)
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg `var' `vars_dc_idv16' scrat0dc_entry no_scrat0dc_entry edddc_entry no_edddc_entry year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSentry_`var'idv16_1flag=e(sample)	
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	xtreg `var' `vars_dc_idv16' scrat0dc_entry no_scrat0dc_entry edddc_entry no_edddc_entry year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSentry_`var'idv16_2flag=e(sample)
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv16' scrat0dc_entry no_scrat0dc_entry edddc_entry no_edddc_entry year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
...
* exit: only 1992 data is usable as we need to t-1 and t+1 data for year t, so no fixed effects. 
	...
	*------------------------------------------------------------*		 
	reg exit `exit_vars_dc_idv5' scrat_entry educ_entry, cluster(num_st)
	* gen TMQSentry_exit_idv5_1flag=e(sample)	
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg exit `exit_vars_dc_idv5' scrat_entry educ_entry year1992 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSentry_exit_idv5_2flag=e(sample)	
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	reg exit `exit_vars_dc_idv6' scrat_entry no_scrat_entry educ_entry no_educ_entry, cluster(num_st)
	* gen TMQSentry_exit_idv6_1flag=e(sample)	
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	xtreg exit `exit_vars_dc_idv6' scrat_entry no_scrat_entry educ_entry no_educ_entry year1992 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSentry_exit_idv6_2flag=e(sample)	
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	reg exit `exit_vars_dc_idv15' scrat0dc_entry edddc_entry, cluster(num_st)
	* gen TMQSentry_exit_idv15_1flag=e(sample)	
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*
	xtreg exit `exit_vars_dc_idv15' scrat0dc_entry edddc_entry year1992 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSentry_exit_idv15_2flag=e(sample)	
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		
	reg exit `exit_vars_dc_idv16' scrat0dc_entry no_scrat0dc_entry edddc_entry no_edddc_entry, cluster(num_st)
	* gen TMQSentry_exit_idv16_1flag=e(sample)	
	outreg using `pathpgs3'MQSentry, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	xtreg exit `exit_vars_dc_idv16' scrat0dc_entry no_scrat0dc_entry edddc_entry no_edddc_entry year1992 if panel_ein==1, fe i(num_ein) cluster(num_st)
...
foreach var of local dep_accredit {
	...
	*------------------------------------------------------------*		 
	areg `var' `vars_dc_idv5' accredit year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSaccre_`var'idv5_1flag=e(sample)
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv5' accredit year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSaccre_`var'idv5_2flag=e(sample)
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv5' accredit year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSaccre_`var'idv5_3flag=e(sample)
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg `var' `vars_dc_idv6' accredit year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSaccre_`var'idv6_1flag=e(sample)
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv6' accredit year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSaccre_`var'idv6_2flag=e(sample)
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv6' accredit year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSaccre_`var'idv6_3flag=e(sample)
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		
	areg `var' `vars_dc_idv15' accredit year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSaccre_`var'idv15_1flag=e(sample)
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv15' accredit year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSaccre_`var'idv15_2flag=e(sample)
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv15' accredit year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSaccre_`var'idv15_3flag=e(sample)
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg `var' `vars_dc_idv16' accredit year1992 year1997, absorb(num_st) cluster(num_st)
	* gen TMQSaccre_`var'idv16_1flag=e(sample)
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	xtreg `var' `vars_dc_idv16' accredit year1992 year1997 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSaccre_`var'idv16_2flag=e(sample)
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg `var' `vars_dc_idv16' accredit year1992 year1997 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
...
* treat exit regression seperately
	...
	*------------------------------------------------------------*		 
	areg exit `exit_vars_dc_idv5' accredit year1992, absorb(num_st) cluster(num_st)
	* gen TMQSaccre_exit_idv5_1flag=e(sample)	
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg exit `exit_vars_dc_idv5' accredit year1992 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSaccre_exit_idv5_2flag=e(sample)	
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg exit `exit_vars_dc_idv5' accredit year1992 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSaccre_exit_idv5_3flag=e(sample)	
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg exit `exit_vars_dc_idv6' accredit year1992, absorb(num_st) cluster(num_st)
	* gen TMQSaccre_exit_idv6_1flag=e(sample)	
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	xtreg exit `exit_vars_dc_idv6' accredit year1992 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSaccre_exit_idv6_2flag=e(sample)	
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg exit `exit_vars_dc_idv6' accredit year1992 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSaccre_exit_idv6_3flag=e(sample)	
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg exit `exit_vars_dc_idv15' accredit year1992, absorb(num_st) cluster(num_st)
	* gen TMQSaccre_exit_idv15_1flag=e(sample)	
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	xtreg exit `exit_vars_dc_idv15' accredit year1992 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSaccre_exit_idv15_2flag=e(sample)	
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg exit `exit_vars_dc_idv15' accredit year1992 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
	* gen TMQSaccre_exit_idv15_3flag=e(sample)	
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*	
	areg exit `exit_vars_dc_idv16' accredit year1992, absorb(num_st) cluster(num_st)
	* gen TMQSaccre_exit_idv16_1flag=e(sample)	
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*
	xtreg exit `exit_vars_dc_idv16' accredit year1992 d_state1-d_state56 if panel_ein==1, fe i(num_ein) cluster(num_st)
	* gen TMQSaccre_exit_idv16_2flag=e(sample)	
	outreg using `pathpgs3'MQSaccredit, se 3aster bracket bdec(3) append
	*------------------------------------------------------------*		 
	xtreg exit `exit_vars_dc_idv16' accredit year1992 if panel_cfn==1, fe i(num_cfn) cluster(num_st)
...
areg exit_zip `exit_vars_dc0' year1992 if year==1987 | year==1992, absorb(num_st) cluster(num_st) 
outreg using `pathpgs3'exit_zip, se 3aster bracket bdec(3) title("Exit Rate") replace  

forvalues i=1/`k' {

	reg exit_zip `exit_vars_dc`i'' if year==1987 | year==1992, cluster(num_st)
	outreg using `pathpgs3'exit_zip, se 3aster bracket bdec(3) append 
	reg exit_zip `exit_vars_dc`i'' year1992 if year==1987 | year==1992, cluster(num_st) 
	outreg using `pathpgs3'exit_zip, se 3aster bracket bdec(3) append 
	areg exit_zip `exit_vars_dc`i'' if year==1987 | year==1992, absorb(num_st) cluster(num_st) 
	outreg using `pathpgs3'exit_zip, se 3aster bracket bdec(3) append 
	areg exit_zip `exit_vars_dc`i'' year1992 if year==1987 | year==1992, absorb(num_st) cluster(num_st) 
	outreg using `pathpgs3'exit_zip, se 3aster bracket bdec(3) append 	
...
forvalues i=1/`k' {
	...
	reg accre_zip_orig `vars_dc`i'' if accre_zip_orig<=1, cluster(num_st)
	outreg using `pathpgs3'accre_zip_orig_rless1, se 3aster bracket bdec(3) append 
	reg accre_zip_orig `vars_dc`i'' year1992 year1997 if accre_zip_orig<=1, cluster(num_st) 
	outreg using `pathpgs3'accre_zip_orig_rless1, se 3aster bracket bdec(3) append 
	areg accre_zip_orig `vars_dc`i'' if accre_zip_orig<=1, absorb(num_st) cluster(num_st) 
	outreg using `pathpgs3'accre_zip_orig_rless1, se 3aster bracket bdec(3) append 
	areg accre_zip_orig `vars_dc`i'' year1992 year1997 if accre_zip_orig<=1, absorb(num_st) cluster(num_st) 
	outreg using `pathpgs3'accre_zip_orig_rless1, se 3aster bracket bdec(3) append 	
