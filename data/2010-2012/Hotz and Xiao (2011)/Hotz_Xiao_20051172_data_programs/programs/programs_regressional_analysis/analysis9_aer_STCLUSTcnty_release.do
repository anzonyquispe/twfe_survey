...
local pathpgs3  	"/rdcprojects/la00296/disclosure/July2007/STCLUST/cnty/" 
local pathdisclose 	"/rdcprojects/la00296/disclosure/July2007/STCLUST/cnty/" 
local pathoutside	"/rdcprojects/la00296/data/outside/" 


log using `pathpgs3'analysis9_aer_STCLUSTcnty.log, replace
} 
...
foreach var of local dep_cty {
	...
	areg `var' `vars_dc0' year1992 year1997, absorb(num_st) cluster(num_st)
...
areg exit_cty `exit_vars_dc0' year1992 if year==1987 | year==1992, absorb(num_st) cluster(num_st) 
outreg using `pathpgs3'exit_cty, se 3aster bracket bdec(3) title("Exit Rate") replace  

forvalues i=1/`k' {

	reg exit_cty `exit_vars_dc`i'' if year==1987 | year==1992, cluster(num_st)
	outreg using `pathpgs3'exit_cty, se 3aster bracket bdec(3) append 
	reg exit_cty `exit_vars_dc`i'' year1992 if year==1987 | year==1992, cluster(num_st) 
	outreg using `pathpgs3'exit_cty, se 3aster bracket bdec(3) append 
	areg exit_cty `exit_vars_dc`i'' if year==1987 | year==1992, absorb(num_st) cluster(num_st) 
	outreg using `pathpgs3'exit_cty, se 3aster bracket bdec(3) append 
	areg exit_cty `exit_vars_dc`i'' year1992 if year==1987 | year==1992, absorb(num_st) cluster(num_st) 
	outreg using `pathpgs3'exit_cty, se 3aster bracket bdec(3) append
...
areg accre_cty_orig `vars_dc0' year1992 year1997 if accre_cty_orig<=1, absorb(num_st) cluster(num_st) 
...
forvalues i=1/`k' {
	...
	reg accre_cty_orig `vars_dc`i'' if accre_cty_orig<=1, cluster(num_st)
	outreg using `pathpgs3'accre_cty_orig_rless1, se 3aster bracket bdec(3) append 
	reg accre_cty_orig `vars_dc`i'' year1992 year1997 if accre_cty_orig<=1, cluster(num_st) 
	outreg using `pathpgs3'accre_cty_orig_rless1, se 3aster bracket bdec(3) append 
	areg accre_cty_orig `vars_dc`i'' if accre_cty_orig<=1, absorb(num_st) cluster(num_st) 
	outreg using `pathpgs3'accre_cty_orig_rless1, se 3aster bracket bdec(3) append 
	areg accre_cty_orig `vars_dc`i'' year1992 year1997 if accre_cty_orig<=1, absorb(num_st) cluster(num_st) 
	outreg using `pathpgs3'accre_cty_orig_rless1, se 3aster bracket bdec(3) append
...
foreach var of local dep_cty2 {
	...
	areg `var' `vars_dc0_nologu5' year1992 year1997, absorb(num_st) cluster(num_st) 
