...
local pathdemog 	"/rdcprojects/la00296/data/outsidemo/zipbundle5/"
local pathpgs3  	"/rdcprojects/la00296/disclosure/July2007/EDU8/zipbundle5/baseline/" 
local pathdisclose 	"/rdcprojects/la00296/disclosure/July2007/EDU8/zipbundle5/baseline/" 
local pathoutside	"/rdcprojects/la00296/data/outside/" 


log using `pathpgs3'analysis9_aer_EDU8zbundle5_baseline.log, replace 
...
* ------------------------
use `pathdemog'zipbundleweights_1990_R05.dta, clear
gen own_neighbor = 0
...
save `pathdemog'zipbundleweights_1990_R05fix.dta, replace

use `pathdemog'zipbundleweights_2000_R05.dta, clear
gen own_neighbor = 0
replace own_neighbor = 1 if marketcentralzip==zipcode
drop if own_neighbor==1 & neighbornumber>0
drop own_neighbor
...
drop marketzipweight
gen marketzipweight = pop / totalmarketpop
save `pathdemog'zipbundleweights_2000_R05fix.dta, replace

gen yeardemocen=2000
append using `pathdemog'zipbundleweights_1990_R05fix.dta
...
areg exit_bundle `exit_vars_dc0' year1992 if year==1987 | year==1992, absorb(num_st) cluster(year_st) 
outreg using `pathpgs3'exit_bundle, se 3aster bracket bdec(3) title("Exit Rate") replace  

forvalues i=1/`k' {

	reg exit_bundle `exit_vars_dc`i'' if year==1987 | year==1992, cluster(year_st)
	outreg using `pathpgs3'exit_bundle, se 3aster bracket bdec(3) append 
	reg exit_bundle `exit_vars_dc`i'' year1992 if year==1987 | year==1992, cluster(year_st) 
	outreg using `pathpgs3'exit_bundle, se 3aster bracket bdec(3) append 
	areg exit_bundle `exit_vars_dc`i'' if year==1987 | year==1992, absorb(num_st) cluster(year_st) 
	outreg using `pathpgs3'exit_bundle, se 3aster bracket bdec(3) append 
	areg exit_bundle `exit_vars_dc`i'' year1992 if year==1987 | year==1992, absorb(num_st) cluster(year_st) 
	outreg using `pathpgs3'exit_bundle, se 3aster bracket bdec(3) append 	
...
forvalues i=1/`k' {
	...
	reg accre_bundle_orig `vars_dc`i'' if accre_bundle_orig<=1, cluster(year_st)
	outreg using `pathpgs3'accre_bundle_orig_rless1, se 3aster bracket bdec(3) append 
	reg accre_bundle_orig `vars_dc`i'' year1992 year1997 if accre_bundle_orig<=1, cluster(year_st) 
	outreg using `pathpgs3'accre_bundle_orig_rless1, se 3aster bracket bdec(3) append 
	areg accre_bundle_orig `vars_dc`i'' if accre_bundle_orig<=1, absorb(num_st) cluster(year_st) 
	outreg using `pathpgs3'accre_bundle_orig_rless1, se 3aster bracket bdec(3) append 
	areg accre_bundle_orig `vars_dc`i'' year1992 year1997 if accre_bundle_orig<=1, absorb(num_st) cluster(year_st) 
	outreg using `pathpgs3'accre_bundle_orig_rless1, se 3aster bracket bdec(3) append 	
...
di "---------------------------------------" 
di 
di "Conc. Ratio for Table 2 - 1987 = " T2cr1987 
di "Conc. Ratio for Table 2 - 1992 = " T2cr1992 
di "Conc. Ratio for Table 2 - 1997 = " T2cr1997 
di 
di "Conc. Ratio for Table 3 - 1987 (first 3 rows)= " T3cr1987acc 
di "Conc. Ratio for Table 3 - 1992 (first 3 rows)= " T3cr1992acc 
di "Conc. Ratio for Table 3 - 1997 (first 3 rows)= " T3cr1997acc 
di "Conc. Ratio for Table 3 - 1987  (last 3 rows)= " T3cr1987wage 
di "Conc. Ratio for Table 3 - 1992  (last 3 rows)= " T3cr1992wage 
di "Conc. Ratio for Table 3 - 1997  (last 3 rows)= " T3cr1997wage 
di 
*di "Conc. Ratio for Table 4 - 1987 = " T4cr1987 
*di "Conc. Ratio for Table 4 - 1992 = " T4cr1992 
*di "Conc. Ratio for Table 4 - 1997 = " T4cr1997 
*di "Conc. Ratio for Table 4 - entry_zip1992 = " T4crentry_zip1992 
*di "Conc. Ratio for Table 4 - entry_zip1997 = " T4crentry_zip1997 
di 
di "Concentration Ratios for Table 50" 
di 
di "Conc. Ratio for Table 50 - lfo:0 xmpt:0 =" T50cr0_0 
di "Conc. Ratio for Table 50 - lfo:1 xmpt:0 =" T50cr1_0 
di "Conc. Ratio for Table 50 - lfo:2 xmpt:0 =" T50cr2_0 
di "Conc. Ratio for Table 50 - lfo:3 xmpt:0 =" T50cr3_0 
di "Conc. Ratio for Table 50 - lfo:4 xmpt:0 =" T50cr4_0 
di "Conc. Ratio for Table 50 - lfo:9 xmpt:0 =" T50cr9_0 
 
 di "Conc. Ratio for Table 50 - lfo:0 xmpt:1 =" T50cr0_1 
*di "Conc. Ratio for Table 50 - lfo:1 xmpt:1 =" T50cr1_1 
*di "Conc. Ratio for Table 50 - lfo:2 xmpt:1 =" T50cr2_1 
 di "Conc. Ratio for Table 50 - lfo:3 xmpt:1 =" T50cr3_1 
 di "Conc. Ratio for Table 50 - lfo:4 xmpt:1 =" T50cr4_1 
 di "Conc. Ratio for Table 50 - lfo:9 xmpt:1 =" T50cr9_1 
 
 di "Conc. Ratio for Table 50 - lfo:0 xmpt:0 =" T50cr0_0_1987 
 di "Conc. Ratio for Table 50 - lfo:1 xmpt:0 =" T50cr1_0_1987 
 di "Conc. Ratio for Table 50 - lfo:2 xmpt:0 =" T50cr2_0_1987 
*di "Conc. Ratio for Table 50 - lfo:3 xmpt:0 =" T50cr3_0_1987 
*di "Conc. Ratio for Table 50 - lfo:4 xmpt:0 =" T50cr4_0_1987 
 di "Conc. Ratio for Table 50 - lfo:9 xmpt:0 =" T50cr9_0_1987 
 
 di "Conc. Ratio for Table 50 - lfo:0 xmpt:1 =" T50cr0_1_1987 
*di "Conc. Ratio for Table 50 - lfo:1 xmpt:1 =" T50cr1_1_1987 
*di "Conc. Ratio for Table 50 - lfo:2 xmpt:1 =" T50cr2_1_1987 
*di "Conc. Ratio for Table 50 - lfo:3 xmpt:1 =" T50cr3_1_1987 
*di "Conc. Ratio for Table 50 - lfo:4 xmpt:1 =" T50cr4_1_1987 
 di "Conc. Ratio for Table 50 - lfo:9 xmpt:1 =" T50cr9_1_1987 
 
 di "Conc. Ratio for Table 50 - lfo:0 xmpt:0 =" T50cr0_0_1992 
 di "Conc. Ratio for Table 50 - lfo:1 xmpt:0 =" T50cr1_0_1992 
 di "Conc. Ratio for Table 50 - lfo:2 xmpt:0 =" T50cr2_0_1992 
*di "Conc. Ratio for Table 50 - lfo:3 xmpt:0 =" T50cr3_0_1992 
*di "Conc. Ratio for Table 50 - lfo:4 xmpt:0 =" T50cr4_0_1992 
 di "Conc. Ratio for Table 50 - lfo:9 xmpt:0 =" T50cr9_0_1992 
 
 di "Conc. Ratio for Table 50 - lfo:0 xmpt:1 =" T50cr0_1_1992 
*di "Conc. Ratio for Table 50 - lfo:1 xmpt:1 =" T50cr1_1_1992 
*di "Conc. Ratio for Table 50 - lfo:2 xmpt:1 =" T50cr2_1_1992 
*di "Conc. Ratio for Table 50 - lfo:3 xmpt:1 =" T50cr3_1_1992 
*di "Conc. Ratio for Table 50 - lfo:4 xmpt:1 =" T50cr4_1_1992 
 di "Conc. Ratio for Table 50 - lfo:9 xmpt:1 =" T50cr9_1_1992 
 
 di "Conc. Ratio for Table 50 - lfo:0 xmpt:0 =" T50cr0_0_1997 
 di "Conc. Ratio for Table 50 - lfo:1 xmpt:0 =" T50cr1_0_1997 
 di "Conc. Ratio for Table 50 - lfo:2 xmpt:0 =" T50cr2_0_1997 
*di "Conc. Ratio for Table 50 - lfo:3 xmpt:0 =" T50cr3_0_1997 
*di "Conc. Ratio for Table 50 - lfo:4 xmpt:0 =" T50cr4_0_1997 
 di "Conc. Ratio for Table 50 - lfo:9 xmpt:0 =" T50cr9_0_1997 
  
 di "Conc. Ratio for Table 50 - lfo:0 xmpt:1 =" T50cr0_1_1997 
*di "Conc. Ratio for Table 50 - lfo:1 xmpt:1 =" T50cr1_1_1997 
*di "Conc. Ratio for Table 50 - lfo:2 xmpt:1 =" T50cr2_1_1997 
*di "Conc. Ratio for Table 50 - lfo:3 xmpt:1 =" T50cr3_1_1997 
*di "Conc. Ratio for Table 50 - lfo:4 xmpt:1 =" T50cr4_1_1997 
 di "Conc. Ratio for Table 50 - lfo:9 xmpt:1 =" T50cr9_1_1997 
 di "----------------------------------------------------------" 
