...
*******************************************************************************
*
*	Merge by 5-digit Zip Code and First X CHARACTERS of Address1
*
*******************************************************************************


* Zip and Address1 matching loop
******************************

forvalues j=$J/38 {
	local i=41-`j'+1
	noi di
	noi di in green "Performing Matching by Zip and first " `i' " characters of Bussiness Address 1 ...."
	...
	*Takes care of P.O. BOX vs P O BOX vs R.D. BOX  vs R D BOX issues
	...
	drop stub* add trimadd lowcaseadd1 addressX5
...
*keep if _merge==3
	...
	*Takes care of P.O. BOX vs P O BOX vs R D BOX issues
	...
	merge zip addressX`i' using `pathnaeyc'naeyc_clean.dta,  _merge(mergebyaddress`i')
	tab mergebyaddress`i'
	keep if mergebyaddress`i'==3
	keep census_rec naeyc_id
	saveold `pathcensus'mergebyaddress1_`i', replace
	keep naeyc_id
	sort naeyc_id
	merge naeyc_id using `pathnaeyc'naeyc_notmatched.dta
	tab _merge
