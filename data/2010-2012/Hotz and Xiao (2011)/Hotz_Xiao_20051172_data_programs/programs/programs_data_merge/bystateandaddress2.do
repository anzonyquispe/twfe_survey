quietly {
* Program Name : bystateandaddress2.do
...
*******************************************************************************
*
*	Merge by STATE and First X CHARACTERS of Address 2
*
*******************************************************************************


* STATE and Address2 matching loop
******************************
...
forvalues j=$J/38 {
	local i=41-`j'
	noi di
	noi di in green "Performing Matching by STATE and first " `i' " characters of Bussiness Address 2...."
	noi di in green "Algorithm started at " c(current_time)
	noi di
	use `pathnaeyc'naeyc_clean_orig.dta , clear
	gen lowcaseadd1=lower(address2)
	...
	sort numstate addressX`i'
	keep  if zip==9999999999999			/* because state + address matching is only for those that do not have zip*/
	keep numstate addressX`i' naeyc_id
	...
	keep addressX`i' year street cfn numstate zip census_rec /* this is for fast testing only */
	sort numstate addressX`i' year
	merge numstate addressX`i' using `pathnaeyc'naeyc_clean.dta,  _merge(mergebyaddressandstate`i')
	tab mergebyaddressandstate`i'
	keep if mergebyaddressandstate`i'==3
	keep census_rec naeyc_id
	saveold `pathcensus'mergebyaddress2andstate_`i', replace
	keep naeyc_id
	sort naeyc_id
	merge naeyc_id using `pathnaeyc'naeyc_notmatched.dta
	tab _merge
