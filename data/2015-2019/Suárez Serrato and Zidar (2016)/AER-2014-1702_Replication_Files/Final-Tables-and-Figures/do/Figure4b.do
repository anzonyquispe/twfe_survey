...
xi: reg g_est L(0/10)x F(1/9)x i.year i.fe_group  [aw=wgt], r cluster(fips_state) 
...
twoway (scatter fig_b fig_t if  fig_t<21 & fig_t>-10, c(l) xline(-0.0, lc(gs11) lp(shortdash)) yline(0, lc(gs11) lp(shortdash))) ///
	...
	ttext(  -8 -5.3 "F-test all leads are 0 has p-value= 0.92", box just(left) size(small) fcolor(white) margin(medsmall)) ///
	ttext(  -8 5.5 "F-test all lags are 0 has p-value= 0.036", box just(left) size(small) fcolor(white) margin(medsmall)) ///
