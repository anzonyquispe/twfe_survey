...
use "$dtapath/CMD/ILS_3_shocks_7_parm_housing.dta", clear  
...
prog cmdestjoint, eclass 
		version 11
		syntax varlist [if] [in]  [aw fw pw iw] [,vce(string) cluster(varname)] gamma(real) alpha(real)
		marksample touse 
	...
	// FEs 
		...
		matrix colnames rb = sigma_F sigma_W eta lambda lambda_h lambda_z epsilon kappa
		matrix colnames rV = sigma_F sigma_W eta lambda lambda_h lambda_z epsilon kappa
		matrix rownames rV = sigma_F sigma_W eta lambda lambda_h lambda_z epsilon kappa

		ereturn post rb rV, obs(`N')

		
		mat rb = (e(b),J(1,11,0))
		mat rV = (e(V),J(8,11,0) \ J(11,19,0) ) 


qui { 
		local eld "(gamma*(_b[epsilon]+1-1/_b[sigma_F])-1)" 		
		local els "((1+_b[eta]-alpha)/(_b[sigma_W]*(1+_b[eta])+alpha)) " 		
		local tw  "((-1/((_b[epsilon]+1)*_b[sigma_F]))/( `els'-`eld' ))" 
		local tr "((1+`els')/(1+_b[eta]))*`tw'"
		local trw "(`tw'-alpha*`tr')"
//	local tpi "(1-(1-gamma)*(_b[epsilon]+1)+ gamma*(_b[epsilon]+1)*`tw' ) "
// For intermediate goods 
		local tpi "((1-0.9*gamma*(_b[epsilon]+1) + gamma*(_b[epsilon]+1)*`tw' ) )"		
		...
		local epsilon = _b[epsilon]
		
		matrix colnames rb = sigma_f sigma_w eta lambda lambda_h lambda_z epsilon kappa micro_els macro_els micro_eld macro_eld tilde_w tilde_r tilde_r_w tilde_pi share_L share_W share_F
		matrix colnames rV = sigma_f sigma_w eta lambda lambda_h lambda_z epsilon kappa micro_els macro_els micro_eld macro_eld tilde_w tilde_r tilde_r_w tilde_pi share_L share_W share_F
		matrix rownames rV = sigma_f sigma_w eta lambda lambda_h lambda_z epsilon kappa micro_els macro_els micro_eld macro_eld tilde_w tilde_r tilde_r_w tilde_pi share_L share_W share_F
...
void mycmd(theta) 
	...
	//	iV = diag(J(1,8,1))
		...
		optimize_init_technique(S, " dfp 10 bfgs 100")
		optimize_init_technique(S, "nm 100")
		optimize_init_nmsimplexdeltas(S,J(1,cols(12),1))
		p = optimize(S)
...
grad=(grad, 0 )
grad=(grad, 0 )
grad=(grad, -(gamma*((a + s_w + eta*s_w)*(eta - a + 1) - s_f*((2*a + 2*s_w + 2*eta*s_w)*(eta - a + 1) + epsilon*(2*a + 2*s_w + 2*eta*s_w)*(eta - a + 1))) + s_f*(eta - a + 1)*(eta + s_w + eta*s_w + 1))/((epsilon + 1)^2*(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w - a*gamma*s_f + eta*gamma*s_w + eta*s_f*s_w - gamma*s_f*s_w - a*epsilon*gamma*s_f - epsilon*gamma*s_f*s_w - eta*gamma*s_f*s_w - epsilon*eta*gamma*s_f*s_w)^2) )
grad=(grad, 0 )
...
grad=(grad, (s_f^2*(eta - a + 1)*(lambda + eta*lambda - a*gamma*lambda_z + a*eta*gamma*lambda_h) + s_f^2*s_w*(eta - a + 1)*(lambda + eta*lambda - gamma*lambda_z - eta*gamma*lambda_z))/(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w - a*gamma*s_f + eta*gamma*s_w + eta*s_f*s_w - gamma*s_f*s_w - a*epsilon*gamma*s_f - epsilon*gamma*s_f*s_w - eta*gamma*s_f*s_w - epsilon*eta*gamma*s_f*s_w)^2 )
grad=(grad, 0 )
...
grad=(grad, 0 )
grad=(grad, 0 )
grad=(grad, (gamma*s_f^2*(eta - a + 1)*(eta - a + a*kappa + a*eta*kappa + 1))/(s_f + a*gamma + eta*s_f - a*gamma*s_f + s_w*(eta + 1)*(gamma + s_f - gamma*s_f - epsilon*gamma*s_f) - a*epsilon*gamma*s_f)^2 )
...
grad=(grad, 0 )
grad=(grad, 0 )
grad=(grad, -(gamma*((a + s_w + eta*s_w)^2 - s_f*(a + s_w + eta*s_w)*(2*a + 2*s_w + 2*a*epsilon + 2*epsilon*s_w + 2*eta*s_w + 2*epsilon*eta*s_w)) + s_f*(a + s_w + eta*s_w)*(eta + s_w + eta*s_w + 1))/((epsilon + 1)^2*(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w - a*gamma*s_f + eta*gamma*s_w + eta*s_f*s_w - gamma*s_f*s_w - a*epsilon*gamma*s_f - epsilon*gamma*s_f*s_w - eta*gamma*s_f*s_w - epsilon*eta*gamma*s_f*s_w)^2) )
grad=(grad, 0 )
...
grad=(grad, -(gamma*s_f^2*(a + s_w + eta*s_w)*(a*lambda_z + lambda_z*s_w - a*eta*lambda_h + eta*lambda_z*s_w) - lambda*s_f^2*(eta + 1)*(s_w + 1)*(a + s_w + eta*s_w))/(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w - a*gamma*s_f + eta*gamma*s_w + eta*s_f*s_w - gamma*s_f*s_w - a*epsilon*gamma*s_f - epsilon*gamma*s_f*s_w - eta*gamma*s_f*s_w - epsilon*eta*gamma*s_f*s_w)^2 )
grad=(grad, 0 )
...
grad=(grad, 0 )
grad=(grad, 0 )
grad=(grad, (gamma*s_f^2*(a + s_w + eta*s_w)*(eta - a + a*kappa + a*eta*kappa + 1))/(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w - a*gamma*s_f + eta*gamma*s_w + eta*s_f*s_w - gamma*s_f*s_w - a*epsilon*gamma*s_f - epsilon*gamma*s_f*s_w - eta*gamma*s_f*s_w - epsilon*eta*gamma*s_f*s_w)^2 )
...
grad=(grad, 0 )
grad=(grad, 0 )
grad=(grad, (gamma*(s_f*((s_w + 1)*(2*a + 2*s_w + 2*eta*s_w) + epsilon*(s_w + 1)*(2*a + 2*s_w + 2*eta*s_w)) - (s_w + 1)*(a + s_w + eta*s_w)) - s_f*(s_w + 1)*(eta + s_w + eta*s_w + 1))/((epsilon + 1)^2*(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w - a*gamma*s_f + eta*gamma*s_w + eta*s_f*s_w - gamma*s_f*s_w - a*epsilon*gamma*s_f - epsilon*gamma*s_f*s_w - eta*gamma*s_f*s_w - epsilon*eta*gamma*s_f*s_w)^2) )
grad=(grad, 0 )
...
grad=(grad, -(gamma*(s_f^2*(lambda_z*s_w + eta*lambda_z*s_w)*(s_w + 1) + a*s_f^2*(s_w + 1)*(lambda_z - eta*lambda_h)) - s_f^2*(s_w + 1)*(lambda + eta*lambda + lambda*s_w + eta*lambda*s_w))/(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w - a*gamma*s_f + eta*gamma*s_w + eta*s_f*s_w - gamma*s_f*s_w - a*epsilon*gamma*s_f - epsilon*gamma*s_f*s_w - eta*gamma*s_f*s_w - epsilon*eta*gamma*s_f*s_w)^2 )
grad=(grad, 0 )
...
grad=(grad, 0 )
grad=(grad, 0 )
grad=(grad, (gamma*(s_f^2*(eta + 1)*(s_w + 1) + a*s_f^2*(s_w + 1)*(kappa + eta*kappa - 1)))/(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w - a*gamma*s_f + eta*gamma*s_w + eta*s_f*s_w - gamma*s_f*s_w - a*epsilon*gamma*s_f - epsilon*gamma*s_f*s_w - eta*gamma*s_f*s_w - epsilon*eta*gamma*s_f*s_w)^2 )
...
grad=(grad, 0 )
grad=(grad, 0 )
grad=(grad, - ((eta + 1)*(s_w + 1))/((epsilon + 1)^2*(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w + eta*gamma*s_w + eta*s_f*s_w)) - (gamma^3*s_f*(a + s_w + eta*s_w)^3)/((s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w + eta*gamma*s_w + eta*s_f*s_w)*(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w - epsilon*(a*gamma*s_f + gamma*s_f*s_w + eta*gamma*s_f*s_w) - a*gamma*s_f + eta*gamma*s_w + eta*s_f*s_w - gamma*s_f*s_w - eta*gamma*s_f*s_w)^2) )
grad=(grad, 0 )
...
grad=(grad, -(gamma*s_f*(a + s_w + eta*s_w)*(lambda + eta*lambda + lambda*s_w - a*gamma*lambda_z + eta*lambda*s_w - gamma*lambda_z*s_w - eta*gamma*lambda_z*s_w + a*eta*gamma*lambda_h))/(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w - a*gamma*s_f + eta*gamma*s_w + eta*s_f*s_w - gamma*s_f*s_w - epsilon*gamma*s_f*(a + s_w + eta*s_w) - eta*gamma*s_f*s_w)^2 )
grad=(grad, 0 )
...
grad=(grad, 0 )
grad=(grad, 0 )
grad=(grad, -(gamma^2*s_f*(a + s_w + eta*s_w)*(eta - a + a*kappa + a*eta*kappa + 1))/(s_f + a*gamma + eta*s_f + gamma*s_w + s_f*s_w - a*gamma*s_f + eta*gamma*s_w + eta*s_f*s_w - gamma*s_f*s_w - a*epsilon*gamma*s_f - epsilon*gamma*s_f*s_w - eta*gamma*s_f*s_w - epsilon*eta*gamma*s_f*s_w)^2 )
...
cluster(state_fips) gamma(.15) alpha(.3)

/* Estimate of net markup
. nlcom 1/(-(_b[epsilon]+1))

       _nl_1:  1/(-(_b[epsilon]+1))

------------------------------------------------------------------------------
        many |      Coef.   Std. Err.      z    P>|z|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
       _nl_1 |   .2699873   .8707233     0.31   0.757    -1.436599    1.976574
------------------------------------------------------------------------------
...
esttab moment* using "$CMD_tablepath/moments_tax_7.csv", ///
...
cmdestjoint dest dpop dadjlrent dadjlwage d_bus_dom2  bartik d_esrate  _Iy* _Ife*  [aw=epop], ///
cluster(state_fips) gamma(.15) alpha(.65)
test (share_F == 0) (share_W == 1) 
estadd scalar p_joint = r(p)
est sto est_2

cmdestjoint dest dpop dadjlrent dadjlwage d_bus_dom2  bartik d_esrate  _Iy* _Ife*  [aw=epop], ///
cluster(state_fips) gamma(.25) alpha(.3)
test (share_F == 0) (share_W == 1) 
estadd scalar p_joint = r(p)
est sto est_3

esttab est_* using "$CMD_tablepath/struct_tax_7.csv", ///
...
forval g = 0.0(.05).5 {
	...
	qui: cmdestjoint dest dpop dadjlrent dadjlwage d_bus_dom2  bartik d_esrate  _Iy* _Ife*  [aw=epop], ///
cluster(state_fips) gamma(`g') alpha(.3)
	...
	replace x_eps = eps if _n == `count'
	replace z_pi =  pi_n*100 if _n == `count'		
	replace z_workers =  w_n*100 if _n == `count'
	replace z_land =  l_n*100 if _n == `count'	
	...
	gen z_markup = (1/(1+1/(x_e))-1)*100

*********	
local bar = .05 
sort y_gamma
twoway (line z_pi y_gamma if z_pv >`bar') (line z_w y_gamma if z_pv >`bar', lpattern(dash)) (line z_l y_gamma if z_pv >`bar', lpattern(longdash)) , graphregion(fcolor(white)) graphregion(color(white)) ytitle("Share of Incidence") xtitle("Output Elasticity of Labor: {&gamma}") legend(order(1 "Firms" 2 "Workers" 3 "Land") rows(1))
graph export "$CMD_graphpath/incidence_7.pdf", replace

twoway (line x_epsilon y_gamma if z_pval >`bar') , graphregion(fcolor(white)) graphregion(color(white)) ytitle("Elasticity of Product Demand: {&epsilon}{superscript:PD}") xtitle("Output Elasticity of Labor: {&gamma}") legend(off)
graph export "$CMD_graphpath/epsilon_7.pdf", replace

sort z_m
twoway (line z_markup x_epsilon if z_pv >0`bar'), graphregion(fcolor(white)) graphregion(color(white)) ytitle("Markup (%)") xtitle("Elasticity of Product Demand: {&epsilon}{superscript:PD}") legend(off)
graph export "$CMD_graphpath/markup_7.pdf", replace
