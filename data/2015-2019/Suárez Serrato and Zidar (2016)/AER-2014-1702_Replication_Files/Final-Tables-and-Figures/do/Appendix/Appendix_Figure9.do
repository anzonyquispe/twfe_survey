...
*Construct Figure
*********************************************************
*do "$dropbox/Local_Econ_Corp_Tax/Programs/Final Tables and Figures/Revision/Profit/do/build_profit_validation_annual.do"
use "$dtapath/Figures/Appendix_Figure9.dta", clear
cd "$append_graphpath"
...
set more off
xi: reg y L(0/10)x F(1/9)x i.year i.fe_group  [aw=wgt], r cluster(fipstate) 
...
*From Long Difference Table, i.e, Table3, yvar(GOS)
gen LDcoeff=6.57
gen LDse=2.56
	
local graph_se=1.96
	
g LDupper=LDcoeff+`graph_se'*LDse
g LDlower=LDcoeff - `graph_se'*LDse
...
twoway (scatter fig_b fig_t if  fig_t<21 & fig_t>-10, c(l) xline(-0.0, lc(gs11) lp(shortdash)) yline(0, lc(gs11) lp(shortdash))) ///
	...
	ylabel(-10 (5) 10) xlabel(-10 (5) 11) ///
	ttext(  -8 -5.3 "F-test all leads are 0 has p-value= 0.84", box just(left) size(small) fcolor(white) margin(medsmall)) ///
	ttext(  -8 5.5 "F-test all lags are 0 has p-value= 0.0005", box just(left) size(small) fcolor(white) margin(medsmall)) ///
...
graph export "Appendix_Figure9.pdf", replace
