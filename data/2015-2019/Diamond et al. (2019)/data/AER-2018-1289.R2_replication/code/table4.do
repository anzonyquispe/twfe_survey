...
prog write_esttab
	...
	esttab using "output/treat_collapse_`smpstub'_`ownstub'.tex", replace ///
		cells(b(star fmt(4)) se(par fmt(4))) ///
		keep(*.treat_year_cat) nobaselevels ///
		refcat(2.treat_year_cat "\emph{Treat\(\times\)Period}", nolabel) ///
		...
		star(* 0.1 ** 0.05 *** 0.01) ///
		label wrap booktabs collabels(none) nonotes ///
		mtitles(`mtitles')
