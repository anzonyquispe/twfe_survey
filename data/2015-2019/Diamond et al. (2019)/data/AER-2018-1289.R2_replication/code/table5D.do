...
prog main
	* robustness check of tenants treatment effect by adding individuals who lived in multi-family buildings with 2-10 units built post 1980 into main control group
	use if is_owner==0 using "data/infutor/infutor_panel_treat_1994_cleaned.dta", clear

	egen avg_nbr_units_treat = max(avg_nbr_units*sample), by(id)
	egen units_assessor_sr_treat = max(units_assessor_sr*sample), by(id)
...
prog reg_main
	...
	global base_sample_treat yr_built_treat>=1900 & yr_built_treat<=1990 & ///
		yrs_at_curr93<=14 & year>=1990 & age_in1993>=20 & age_in1993<=65 & treat==1 & use_code_treat==3
	global base_sample_control1 yr_built_treat>=1900 & yr_built_treat<=1990 & ///
		yrs_at_curr93<=14 & year>=1990 & age_in1993>=20 & age_in1993<=65 & treat==0 & use_code_treat==3
	global base_sample_control2 yr_built_treat>=1900 & yr_built_treat<=1990 & ///
		yrs_at_curr93<=14 & year>=1990 & age_in1993>=20 & age_in1993<=65 & ///
		treat==0 & use_code_treat==4 & (avg_nbr_units_treat<=10 | units_assessor_sr_treat<=10)
	global base_sample_control ($base_sample_control1 | $base_sample_control2)

	eststo clear

	* Treament effects on multi-family res (5+) units.
	loc sample_smallmulti ($base_sample_treat | $base_sample_control)
...
prog write_esttab
	...
	esttab using "output/treat_collapse_`smpstub'_`ownstub'_altcontrol2.csv", replace wide plain ///
