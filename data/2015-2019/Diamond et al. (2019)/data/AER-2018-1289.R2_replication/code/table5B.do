...
prog main	
	//robustness check of tenants treatment effect using census tract by year fixed effects
	use if is_owner==0 using "data/infutor/infutor_panel_treat_1994_cleaned.dta", clear
	destring census_tract_treat, replace
...
prog reg_treat_year_cat
	...
	eststo: reghdfe `varlist' io0b1.treat_year_cat `if', absorb(i.year_cat#i.yrs_at_curr93 i.year_cat#i.census_tract_treat id) vce(cluster `clustvar')
...
prog write_esttab
	...
	esttab using "output/treat_collapse_`smpstub'_`ownstub'_tractfe.csv", replace wide plain ///
