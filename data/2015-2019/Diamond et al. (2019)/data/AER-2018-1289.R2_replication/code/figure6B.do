...
prog gen_var
	* Merge in the outcomes of individual's 1993 census tract
 	* We also have _m==-1 due to matchvar==missing
 	mmerge tractid_treat year using "data/census/ltdb_std_all_2010_adjusted.dta", ///
 		type(n:1) umatch(tractid year) unmatched(master) missing(nomatch) uname(tract93_)
 	drop if _m==-1
 	drop _m

 	* Generate integer group for census tract to be used in interaction terms in regressions
 	egen tract93_id = group(tractid_treat)
	
	* Hold the census tracts of treatment group fixed, only let the control group move
	foreach var of varlist mrent-p10yrs {
		replace `var' = tract93_`var' if treat==1
	}
end
...
prog plot_coef
	...
	graph export "output/treat_`smpstub'_`ownstub'_tractvar`postfix'_semistatic.pdf", replace  
end
