function F = phat1trans( x,k, sig, I_1, b, U_wt, trat_wt ,beta,alpha,T,g, omega_h,tau0rat_wt)
...
F(1)=P1_T^(-k) - ( (I_1*tau0rat_wt+(1-I_1)*((1-beta^(T+1))+beta^(T+1)*P1_T^(-k+sig-1)*U_h^(k-sig+1)))/(I_1*U_wt*((1+u)^(1-k/(sig-1)))+(1-I_1).*U_h.^(k-sig+1)));
