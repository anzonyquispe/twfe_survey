...
* construct nu for each caste
* CUSTOM - choose whether you would like to use income gain or adjusted income gain for calculating nu
by stcst, sort: egen inc_change_caste = mean(income_change)
by stcst, sort: egen mig_caste = mean(mig1)

* CUSTOM - specify the default pnu_caste for castes with no migrant household
by stcst, sort: gen pnu_caste = -ln(mig_caste/2)/ln(1 + inc_change_caste) if mig_caste !=0
replace pnu_caste =90 if mig_caste ==0
...
collapse  (count) pop=pminc (mean) pminc pvarinc mig1 pnu_caste (sum) notmig (mean) ptcon = ptconr4 cutoff* inc_change*, by(stid stcst inccat)
...
count
order pop inccat pminc pvarinc mig1 stcst stid pnu_caste notmig plam absinc_class
sort stid stcst inccat
disp _N/(inccat[_N]+1)

outfile pop inccat pminc pvarinc mig1 stcst stid pnu_caste notmig plam absinc_class using data, replace wide
