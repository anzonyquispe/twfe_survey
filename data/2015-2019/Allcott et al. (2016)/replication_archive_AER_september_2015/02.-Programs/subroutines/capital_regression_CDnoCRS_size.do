...
// Data file
use panelgroup year lshare_final mshare_final eshare_final lnY lnM lnL lnK lnE nic87 nic87_super quintile using "$work/temp2.dta" , clear //adjusted SOC
drop if lshare_final+mshare_final >.95 //added SOC
/*
...
gen beta_m=mshare_final
gen beta_l=lshare_final
g beta_e = eshare_final

// Transformed output net of labor and material contribution
gen tdlnY=lnY-beta_l*lnL-beta_m*lnM-beta_e*lnE
...
levelsof quintile if snic2==`j', local(quint)
foreach k of local quint {
dis "RUNNING FOR `j' `j' `j' `j'"
global dropglobal "snic2!=`j' & quintile!=`k'"
display "$dropglobal"
bootstrap , reps(50) seed(12345): abond_capital
estimates store estnic`k'`i'
replace betak = _b[beta_k] if snic2== `j' & quintile==`k'
replace sebetak = _se[beta_k] if snic2== `j'  & quintile==`k'
}
}
...
disp("end")
drop snic2
keep betak nic87_super quintile
duplicates drop
save "$work/betak_size", replace
