...
use "$data/IOtables/12Oct_IO8990", clear
order  commodity iottsector
rename iottsector buyer
reshape long io, i( commodity buyer) j(seller)
...
use `master', clear
merge m:1 buyer seller using `iomatrix_iott', keep(3) nogen
drop seller buyer
rename nic3buyer src
reshape wide inputshare, i(nic3seller) j(src)
...
matrix C=A*T
svmat C, name(output_lambda)
drop lambda_final
rename output_lambda output_lambda

save "$work/output elec intensity.dta", replace
