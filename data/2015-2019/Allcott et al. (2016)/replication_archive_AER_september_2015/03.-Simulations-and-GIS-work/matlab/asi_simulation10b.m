...
function asi_simulation10b(shortagefile,p_e_g,p_e_s)
% Version that uses median lambda in an industry
thedirectory=['cd simulation_asi_inputs_dec2014'];

infilename=['ASIpanelfields_' int2str(shortagefile) 'pct.csv']

outfilename=['ASI_Prediction' int2str(shortagefile) 'pct.csv'] 
...
[Y_shortage_prime, Y_no_shortage_prime,M_shortage, M_no_shortage,L_shortage, L_no_shortage,E_self_shortage,E_self_no_shortage,E_grid_shortage,E_grid_no_shortage,exitflag]=...
    optimal_output2(alpha_e(i),alpha_m(i),alpha_l(i),alpha_k(i),OMEGA(i),K(i),p(i),p_m(i),p_l(i),p_e_g*1.01,p_e_s,delta_psp(i),no_generator);
