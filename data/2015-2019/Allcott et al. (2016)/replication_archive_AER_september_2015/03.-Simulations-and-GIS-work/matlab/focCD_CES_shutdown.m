function res=focCD_CES(X,alpha_m,alpha_l,alpha_k,alpha_e,OMEGA,K,p,p_m,p_l,p_e_g,delta,sigma)
...
Y_ittau_G=p*OMEGA*exp(X(1))^alpha_l * exp(X(2))^alpha_m * K^alpha_k*exp(X(3))^alpha_e;
...
foc_e_g = ((1/sigma)*Yit^exponenter *sigma*alpha_e * Y_ittau_G.^sigma / exp(X(3)) -p_e_g);
