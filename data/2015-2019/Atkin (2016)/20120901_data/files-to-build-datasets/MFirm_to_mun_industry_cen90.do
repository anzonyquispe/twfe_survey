...
/**
May 2013
This has erics maq definitions in as well
**/
...
gen registro=regexr(firmid,"_..$","")
merge m:1 registro using "`dirmaq'imss_emime_match_eric.dta" , generate(_mergemaq)
gen MaqEriccon=(con==1)
gen MaqEric=(_mergemaq==3)
drop _mergemaq registro con
...
*egen singleyearfirm=max(agefirm)
*replace singleyearfirm=0 if singleyearfirm>0 &  singleyearfirm!=.
*this takes value 1 if firm only open for one year.
...
*this calculates the number of expansions/contractions of the same firm.
gen  nfifty=1 if abs(deltaemploy)>=50 & deltaemploy!=.
gen  all=1 if deltaemploy!=.
egen cfifty=count(nfifty) , by(firmid)
egen call=count(all) , by(firmid)
gen rfifty=1 if (cfifty/call)<0.5
replace rfifty=0 if (cfifty/call)>=0.5
*so this measure only includes expansions where firm does not expand more than 50 in half or more periods. 

gen deviation=0
replace deviation=1 if call==1
replace deviation=1 if deltaemploy!=. & call>1 & sddemploy!=. & meandemploy!=. & (deltaemploy>(meandemploy+sddemploy) | deltaemploy<(meandemploy-sddemploy))
*so this measure only includes expansions more than 1 sd away from firm mean.

drop nfifty all cfifty call
...
gen delta`type'Imaqe=delta`type'*MaqEric
gen `type'Imaqe=`type'*MaqEric
gen delta`type'Imaqec=delta`type'*MaqEriccon
gen `type'Imaqec=`type'*MaqEriccon
gen delta`type'Irfif=delta`type'*rfifty
gen delta`type'Idev=delta`type'*deviation
}
...
save  "`scratch'firmdata_firmtotals.dta", replace

*so this has 1985 data... where did it go?
...
local frog=`n'+`split'
use if group>=`n' & group<`frog' using "`scratch'firmdata_firmtotals.dta", clear
...
foreach type in employ male female firm employImaq maleImaq femaleImaq employImaqe maleImaqe femaleImaqe employImaqec maleImaqec femaleImaqec { 
foreach pre in ""  { 
...
save  "`scratch'firmdata_munindtotals`n'.dta", replace
...
use "`scratch'firmdata_munindtotals1.dta", clear
erase "`scratch'firmdata_munindtotals1.dta"
local rmin=1+`split'

forval n=`rmin'(`split')`rmax' {
append using "`scratch'firmdata_munindtotals`n'.dta"
erase "`scratch'firmdata_munindtotals`n'.dta"
}
...
sort mungrupo year
foreach vary in employ male female employImaq maleImaq femaleImaq  employImaqe maleImaqe femaleImaqe employImaqec maleImaqec femaleImaqec {
foreach cut in  $cutlist {
...
save  "`scratch'firmdata_munindtotals.dta", replace
...
**so this code may generate too many variabels as I reshape this wide. will drop some of the ones i dont use.
...
use "`scratch'firmdata_munindtotals.dta", clear


sort grupo 
merge grupo using "`dir'IMSS_Hcode_David_imss2cen90.dta", _merge(_mergehcode) nokeep keep(imms2cen90)
drop grupo
rename imms2cen90 grupo
...
*drop some stuff I never use here... or else too many variabels
drop *Isize* *Ivar* *Icv* *Iage* *Imean*
*these were interacted with size variance etc
...
save  "`scratch'firmdata_hcodecen90indtotals.dta", replace
...
*second hcode industry codes: this is only for teh newind merge type
*------------------------------------------------------------


foreach typer in "cen90" {

use   "`scratch'firmdata_hcode`typer'indtotals.dta", clear
...
compress
sort year muncenso
save "`scratch'newind`typer'_simple_skill.dta", replace
...
compress
sort year muncenso
save "`scratch'newind`typer'_simpleZM_skill.dta", replace
