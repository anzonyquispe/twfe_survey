...
local listsexcatinitial "
indtdeltamale90ratio50_* indtdeltafem90ratio50_*
inddeltamale90i1ratio50_* inddeltafem90i1ratio50_*
indtdeltamale90i1ratio50_* indtdeltafem90i1ratio50_*
indmale86i1ratio00_* indfem86i1ratio00_*
 ";
...
*normal, migrant, experience and include informal: lets do all of these seperately
#delimit ;
local listinteract "`listsexcatinitial' " ; 
#delimit cr
...
*`listsexcatinitial' `listcoefskill1' `listcoefskill2' `listcoefskill3' `listsexcathirefire' `listsexcatannual'
local listsex: subinstr local listsexcatinitial "*" "", all




foreach thing in deltaemp  {
foreach interact in `listinteract' `listsex' {
...
cap {
sort muncenso
merge m:1 muncenso using "${workdir}Skill_Wage_Cohort_percentiles1990_by_Mun_industry_wide_bothsexes${herf2}_3cats_wages_renewexp.dta", generate(_merge9999)  keepusing(muncenso `listskill3cat190_wages'  ) 
drop _merge9999
}
...
foreach interact in `listsex' {
foreach thing in $listfirm  {

local thingshort=subinstr("`thing'","delta","d",.)
local listsexname "`listsexname' `thingshort'`abbrev'`ends'"
...
forval n=100/999 {
cap gen `thingshort'`abbrev'`ends'`cut'`n'=`thing'`cut'`n'*`interact'`n'
}
...
*ok so now I have sex variables i can stick in instead of deltaemp
...
foreach thing in $listfirm `listsexname' {
...
cap drop dempdm*ra?0f*
cap drop dempdf*ra?0m*
cap drop demptdm*ra?0f*
cap drop demptdf*ra?0m*
...
cap renpfix dempdm dmale
cap renpfix demptdm dmalet
cap renpfix dempdf dfem
cap renpfix demptdf dfemt
