/*==============================================================================
  classify_sketch2_test.do
  Do-file equivalent of classify_sketch2.ado for manual testing.

  Usage:
    1. Set `wave' and `folder' in the paper-selection block below.
    2. Adjust Y G T D locals if your variable names differ.
    3. do "$twfe_root/classify_sketch2_test.do"
    Data is loaded from $twfe_root/panels_GTD/<wave>/<folder>/panel_GTD.dta

  The working copy is preserved/restored so your original data is unchanged.
  Comment out `preserve` and `restore` to inspect intermediate variables.

  Final result stored in globals: $cs2_design  $cs2_design_code
                                  $cs2_estimator  $cs2_had_stayers
==============================================================================*/

* --------------------------------------------------------------------------
* Set variable names here (defaults match panel_GTD.dta convention)
* --------------------------------------------------------------------------
...
* --------------------------------------------------------------------------
* Paper to test — set wave and folder name here
* --------------------------------------------------------------------------
local wave   "2010-2012"
local folder "Zhang and Zhu (2011)"

global panels_GTD "$twfe_root/panels_GTD"
use "$panels_GTD/`wave'/`folder'/panel_GTD.dta", clear
...
di _n as text "===== classify_sketch2_test ====="
di    as text "  Y=`Y'  G=`G'  T=`T'  D=`D'"

preserve

* --------------------------------------------------------------------------
* 0. Prepare working dataset
* --------------------------------------------------------------------------
keep `Y' `G' `T' `D'
    ...
    * -----------------------------------------------------------------------
    * 1. Normalize T to consecutive integers (always done first)
    * -----------------------------------------------------------------------
    tempvar __Tnorm
...
//     drop `__d1'
    ...
    * -----------------------------------------------------------------------
    ...
    qui bysort `G': egen `__DFg' = min(`__dFgtmp')
    drop `__dFgtmp'
    ...
    * V(Fg | Fg <= Tmax): variance of Fg among switchers, group-level
    qui sum `__Fg' if `__first' & `__Fg' <= `Tmax'
    local n_switchers = r(N)
    local V_Fg_cond   = cond(missing(r(Var)), 0, r(Var))
    ...
    * V(D_g1): variance of baseline treatment, group-level (all groups)
    ...
    local V_Dg1 = cond(missing(r(Var)), 0, r(Var))
    ...
    * Binary: D takes only {d_min, d_max} across ALL observations
    qui sum `D'
    local d_max = r(max)
    local d_min = r(min)
    tempvar __bchk
    ...
    * Absorbing: after Fg, D stays at D_Fg
    tempvar __abs_chk
    qui gen `__abs_chk' = (`D' == `__DFg') if `T' > `__Fg' & !missing(`D')
    qui sum `__abs_chk'
...
if `__dyn_ok' {
    ...
    * --- Condition 6: SSFSD ------------------------------------------------
        ...
        di _n as result ///
            "Design SSFSD: did_multiplegt_dyn ran; conditions 3-5 not met."
...
* --------------------------------------------------------------------------
* Store results in globals (survive restore)
* --------------------------------------------------------------------------
global cs2_design      "`__design'"
global cs2_design_code  `__design_code'
global cs2_estimator   "`__estimator'"
global cs2_had_stayers  `__had_stayers'
...
di    as result "  had_stayers    : `__had_stayers'"
di    as result "========================================="
di    as text   "  (also in globals: cs2_design  cs2_design_code"
di    as text   "                    cs2_estimator  cs2_had_stayers)"
