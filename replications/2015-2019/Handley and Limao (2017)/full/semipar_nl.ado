...
*version 1.2
program define semipar_nl, eclass

*based on semipar and changed to remove scatter from graphs and to overlay the OLS fit*
...
lpoly `res2' `nonpar' `wgt' if `touse', degree(`degree') nograph at(`nonpar') kernel(`kernel')   saving(`generate', replace)
}
...
if "`ci'"!="" {
	...
	if "`generate'"!="" {
	twoway (lpolyci `res2' `nonpar' `wgt' if `touse', bwidth(`b1') kernel(`kernel') degree(`degree') level(`level')) (line yplot `nonpar'  if `touse') if `touse', legend(off) title(`title') ytitle(`ytitle') xtitle(`xtitle')  saving(`generate', replace)
	...
	if "`generate'"!="" {
	twoway (lpolyci `res2' `nonpar' `wgt' if `touse', bwidth(`b1') kernel(`kernel') degree(`degree')  fcolor(none) alcolor(none)) (line yplot `nonpar'  if `touse') if `touse', legend(off) title(`title') ytitle(`ytitle') xtitle(`xtitle')   saving(`generate', replace)
	}
