/*==============================================================
  Replicacion: Hirano et al. (2022) JOP
  Equivalente a: papers/Hirano_et_al_2022.R

  Y:      ln_x_cand_all
  D:      prim_sw
  Unit:   state (string -> encode a numerico)
  Time:   year
  FE paper original: state + year
  FE replicacion:    state + year
  Cluster: state
  Controles: ninguno

  NOTA: Filtrar solo anos pares (como en R: df <- df[which(df$year%%2==0),])
  NOTA: CS Never Treated NO DISPONIBLE para este paper
...
use "data/polisci/data_hirano.dta", clear

* Filtrar solo anos pares (como en R)
keep if mod(year, 2) == 0

* state: si es string, encode; si es numerico, gen unit_num
cap confirm string variable state
if _rc == 0 {
    * es string
    encode state, gen(unit_num)
    drop state
}
else {
    * es numerico
    egen unit_num = group(state)
}

* year: convertir a enteros consecutivos (como en R: as.numeric(factor(year)))
* CRITICO: Hirano solo tiene anos pares, usar group() para evitar gaps
egen year_num = group(year)

* Variables principales
local Y "ln_x_cand_all"
local D "prim_sw"
...
*==============================================================
* 1. TWFE REPORTADO (FE del paper: state + year)
*    reghdfe ln_x_cand_all prim_sw, absorb(unit_num year_num) cluster(unit_num)
*==============================================================
...
* 6b. Never Treated (NO DISPONIBLE para este paper)
scalar cs_nv_est = .
scalar cs_nv_se  = .

di ""
di "=== 6b. CS (Never Treated) === NOT AVAILABLE"
...
save "$path/replications/polisci/output/hirano_estimates.dta", replace

di ""
di "============================================"
di "  RESULTADOS FINALES: Hirano et al. (2022)"
di "============================================"
