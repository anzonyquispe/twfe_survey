/* Test: SA para Esberg - pre capado a -12, post COMPLETO (0 a max) */
...
* Pre capado a -12, post COMPLETO
summ rel_time if treat == 1
local tmin = max(r(min), -12)
local tmax = r(max)
di "Rango: `tmin' a `tmax' (pre capado, post completo)"
...
di "Numero de dummies: `: word count `relvars''"
di "=== Comenzando eventstudyinteract ==="
...
* ATT ponderado sobre TODOS los post
local n_post = `tmax' + 1
...
di ""
di "=== SUN & ABRAHAM (pre -12, post completo) ==="
di "ATT: " sa_est
di "SE:  " sa_se
