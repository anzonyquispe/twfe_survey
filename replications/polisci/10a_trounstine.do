/*==============================================================
  Replicacion: Trounstine (2020) APSR
  Equivalente a: papers/Trounstine_2020.R

  Y:      pctnhwht_cityi
  D:      fairhousingimp_v1
  Unit:   geo_id2 (string -> encode a numerico)
  Time:   year
  FE paper original: geo_id2 + year
  FE replicacion:    unit_num + year_num
  Cluster: unit_num
  Controles: ninguno

  SPECIAL: drop.period=T (eliminar unidades con 1 solo periodo)
...
use "data/polisci/data_trounstine.dta", clear

* geo_id2: convertir a numerico si es string
cap confirm string variable geo_id2
if !_rc {
    encode geo_id2, gen(unit_num)
    drop geo_id2
}
else {
    egen unit_num = group(geo_id2)
}

* year: crear year_num como factor numerico (equivalente a as.numeric(factor()))
egen year_num = group(year)

* Variables principales
local Y "pctnhwht_cityi"
local D "fairhousingimp_v1"
...
*==============================================================
* 1. TWFE REPORTADO (FE del paper: geo_id2 + year)
*    R: feols(pctnhwht_cityi ~ fairhousingimp_v1 | geo_id2 + year)
*==============================================================
...
* 2a-bis. drop.period: eliminar unidades con un solo periodo observado
bysort unit_num: gen _nperiods = _N
count if _nperiods == 1
di "Obs de unidades con 1 solo periodo: " r(N)
drop if _nperiods == 1
drop _nperiods
...
save "$path/replications/polisci/output/trounstine_estimates.dta", replace

di ""
di "============================================"
di "  RESULTADOS FINALES: Trounstine (2020)"
di "============================================"
