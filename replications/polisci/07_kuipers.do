/*==============================================================
  Replicacion: Kuipers & Sahn (2023) APSR
  Equivalente a: papers/Kuipers_Sahn_2023.R

  Y:      govt_prop
  D:      treat
  Unit:   city (string o numerico -> unit_num)
  Time:   YEAR
  FE paper original: city + YEAR
...
use "data/polisci/data_kuipers.dta", clear

* Convertir unit a numerico
cap confirm string variable city
if _rc == 0 {
    encode city, gen(unit_num)
}
else {
    egen unit_num = group(city)
}
...
local Y "govt_prop"
local D "treat"
local controls "pop_prop"

di "=== DATOS CARGADOS ==="
describe
summarize `Y' `D' `controls' year_num unit_num


*==============================================================
* 1. TWFE REPORTADO (FE del paper: city + YEAR)
*    reghdfe govt_prop treat pop_prop, absorb(unit_num year_num) cluster(unit_num)
*==============================================================

reghdfe `Y' `D' `controls', absorb(unit_num year_num) cluster(unit_num)
...
* Correr Sun & Abraham (con controles)
...
* 6a. Not Yet Treated (con controles)
scalar cs_ny_est = .
scalar cs_ny_se  = .
cap noi csdid `Y' `controls', ivar(unit_num) time(year_num) gvar(FirstTreat) notyet method(reg)
if _rc == 0 {
    estat simple
...
* 6b. Never Treated (con controles)
scalar cs_nv_est = .
scalar cs_nv_se  = .
cap noi csdid `Y' `controls', ivar(unit_num) time(year_num) gvar(FirstTreat) method(reg)
if _rc == 0 {
    estat simple
    matrix b_cs2 = r(b)
    matrix V_cs2 = r(V)
...
cap noi did_imputation `Y' unit_num year_num FT_imp, controls(`controls') minn(0) nose
if _rc == 0 {
...
save "$path/replications/polisci/output/kuipers_estimates.dta", replace

di ""
di "============================================"
di "  RESULTADOS FINALES: Kuipers & Sahn (2023)"
di "============================================"
